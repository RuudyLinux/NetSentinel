...
class ConfigNode:
    ...
    Vendor-neutral by construction: nesting is driven purely by leading-whitespace
    indentation, which both Cisco IOS (`line vty 0 15` / `exec-timeout 10 0`) and
    FortiOS (`config system global` / `set admintimeout 15`) use consistently in
    their exported configuration text.
    """
...
def _is_noise(line: str, comment_chars: str) -> bool:
    stripped = line.strip()
    return not stripped or stripped.startswith(tuple(comment_chars))


def parse_indented(text: str, *, comment_chars: str = "!") -> ConfigTree:
    """Build an indentation tree from a whitespace-indented configuration.

    `comment_chars` is the set of characters that mark a comment line when they
    lead the (stripped) line: Cisco IOS uses `!`, FortiOS uses `#`.
    ...
    for lineno, raw in enumerate(text.splitlines(), start=1):
        if _is_noise(raw, comment_chars):
            continue
